...
5.2. Inside any `schemas/` folder, split models into multiple files by category when that makes sense. Do not pile unrelated Pydantic models into one large schema file. Prefer files like `tool_errors.py`, `web_search.py`, `open_url.py`, or `http_fetch.py`, and remove misplaced top-level schema folders like `backend/app/schemas` by moving each schema file into its correct feature-level `schemas/` folder.
